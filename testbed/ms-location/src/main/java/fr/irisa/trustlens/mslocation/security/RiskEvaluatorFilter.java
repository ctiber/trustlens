    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if (!deviceDecision.allowed()) {
                log.warn("[RiskFilter] DENY device — user={} reason={}", userId, deviceDecision.reason());
                response.sendError(HttpServletResponse.SC_FORBIDDEN, deviceDecision.reason());
                return;
            }
            ...
            if (latParam != null && lonParam != null) {
                try {
                    ...
                    var travelDecision = riskEvaluator.evaluateLocation(userId, lat, lon);
                    if (!travelDecision.allowed()) {
                        log.warn("[RiskFilter] DENY travel — user={} reason={}", userId, travelDecision.reason());
                        response.sendError(HttpServletResponse.SC_FORBIDDEN, travelDecision.reason());
                        return;
                    }
                } catch (NumberFormatException ex) {
                    response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid lat/lon parameters");
                    return;
                }
            }
